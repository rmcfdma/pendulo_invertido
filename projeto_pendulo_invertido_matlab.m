...
ci  = [0 0 pi/180 0]; % pi/180 rad = 1 grau 
ci2 = [0 0 pi/180 0]; % pi/180 rad = 1 grau 
...
%% 19 - Sistema em malha fechada em s
sysmf = feedback(sys,[K(1) K(3)]); % Sistema em malha fechada
%% 20 - Cáculo da Largura de Banda
...
%% 21 - Cálculo do Período de Amostragem pelo Cálculo Numerico, Teorema de Nyquist e valores usuais
...
S = 2*I*vpa(inv(abs(diag(eig(B*K-A)))),6); % Faz 2*I dividido por uma matriz diagonal que contém o módulo dos autovalores 
...
dt = (2*pi)/(omega_s_escolhido);   % Príodo de amostragem escolhido

%% 22 - Definindo os parâmetros probabilísticos
...
px = eye(4,4);                     % Criando a matriz de covariância entre o sistema e as estimativas          
P = eye(4,4);                      % Criando a matriz de covariância entre o sistema e as estimativas               
Px = diag(px);                     % Criando o vetor acumulador de diagonal principal das matrize covariância
...
%% 23 - Variáveis utilizadas no Método Iterativo
...
%% 24 - Sistema não-linear (real e sua esperança) sem controle e (com e sem) ruido
% for i = 0:dt:tempo_simulação
%       u = 0;                                       % Lei de controle com acurácia(erro sistemátíco) como ruído 
%       u2 = 0                                       % Lei de controle com acurácia(erro sistemátíco) como ruído 
%       x = x + f_h(u,x(2),x(3),x(4))*dt;            % Estados do sistema (Método de Euler)
%       x2 = x2 + (f_h(u2,x2(2),x2(3),x2(4))+ w)*dt; % Estados do sistema (Método de Euler)
%       y = h_h(x(1),x(3));                          % Adicionando o ruído na medição
%       y2 = h_h(x2(1),x2(3))+v;                     % Adicionando o ruído na medição
...
%% 25 - Sistema não-linear sem controle e sem ruido
...
%% 26 - Sistema não-linear sem controle e sem ruido
% for i = 0:dt:tempo_simulacao
%       u = 0;                             % Lei de controle
%       x = x + f_h(u,x(2),x(3),x(4))*dt;  % x[n+1] = x[n]+f(x[n],u[n])*dt
...
%% 27 - Sistema não-linear com controle LQR e sem ruídos
for i = 0:dt:tempo_simulacao
    ...
    t = [t i];                           % Acumulando o Tempo
    draw(x,2,l,l_carrinho,h_carrinho);                           % Animação
end

%% 28 - Sistema não-linear com controle LQR e com ruido apenas no sistema
% for i = 0:dt:tempo_simulacao
%     u = -K*x;                               % Lei de controle
...
%% 29 - Sistema não-linear com controle LQR e com ruido no sistema e no sensor
% for i = 0:dt:tempo_simulacao
%     u = -K*x;                                  % Lei de controle
%     x = x + (f_h(u,x(2),x(3),x(4)) + w)*dt;    % x[n+1] =  x[n]+f(x[n],u[n])*dt com ruído w
...
%% 30 - Sistema linearizado sem controle e sem ruido  
% for i = 0:dt:tempo_simulacao
%     u = 0;                      % Lei de controle  
...
%% 31 - Sistema linearizado com controle LQR e sem ruido 
...
%% 32 - Sistema linearizado com controle LQR e com ruido aditivo no sistema
% for i = 0:dt:tempo_simulacao
%     u = -K*x;                          % Lei de controle 
...
%% 33 - Sistema linearizado com controle LQR e com ruido no sistema e no sensor
...
%% 34 - KF - Filtro de Kalman seguindo o sistema linearizado sem ruido
...
%     % draw(xhat,2)                     % Animação
% end

%% 35 - KF - Filtro de Kalman seguindo o sistema linearizado com ruido no sistema
...
%% 36 - KF - Filtro de Kalman seguindo o sistema linearizado com ruido no sistema e no sensor
...
%% 37 - LQG - Filtro de Kalman + LQR sem ruido
...
%% 38 - LQG - Filtro de Kalman + LQR com ruído no sistema
...
%% 39 - LQG - Filtro de Kalman + LQR com ruído no sistema e nos sensores
...
%% 40 - EKF - Filtro de Kalman Estendido sem ruídos
...
%     U = [U u];                             % Acumulando a lei de controle
%     t = [t i];                             % Acumulando o tempo 
%     % draw(xhat,2)                         % Animação - draw(estimatva atualizada,tamanho da haste)
% end


%% 41 - EKF - Filtro de Kalman Estendido com ruído aditivo no sistema
...
%% 42 - EKF - Filtro de Kalman Estendido com ruidos aditivos no sistema e nos sensores
...
%% 44 - EKF com LQR com ruído no sistema
...
%% 45 - EKF com LQR e ruídos no sistema e no sensor
...
%% 46 - Função Densidade de Probabilidades
...
%% Plot das saídas dos sistemas não filtrados
% plot(t,Y')
% legend('Posiçãodo carrinho','Orientação do pêndulo')

%% Plot das saídas dos sistemas filtrados
% plot(t,Y',t,Yhat')
% legend('Posiçãodo carrinho real','Orientação do pêndulo real','Posição do carrinho estimada','Orientação do pêndulo estimada')

% function graph = plotar_um_grafico(xdata,x_texto,ydata,y_texto,width_linha,legenda)
% function graphs = plotar_dois_graficos(x_data1,y_data1,x_data2,y_data2,width_linha,x_label,y_label,titulo,legenda_1,legenda_2)


% plotar_um_grafico(t,'tempo-s',Y','Posição do Carrinho - m',2,'Posição')

% subplot(1,2,1)
% plotar_dois_graficos(t,Y2(1,:)',t,Y(1,:)',2,'Tempo - s','Posição do Carrinho - m','Gráfico da posição do Carrinho (modelo não-linear) vs. Tempo','Sistema Real','Média (Esperança)')
% 
% subplot(1,2,2)
% plotar_dois_graficos(t,Y2(2,:)',t,Y(2,:)',2,'Tempo - s','Ângulo da Haste - º','Gráfico do Ângulo da Haste (modelo não-linear) vs. Tempo','Sistema Real','Média (Esperança)')

%% Plotandos os Estados
% subplot(2,2,1)
% plotar_um_grafico(t,'Tempo-s',X(1,:)','Posição do Carrinho - m',2,'Posição do Carrinho vs. Tempo ')
% subplot(2,2,2)
% plotar_um_grafico(t,'Tempo-s',(180/pi)*X(3,:)','Ângulo da Haste - º',2,'Ângulo da Haste vs. Tempo')
% subplot(2,2,3)
% plotar_um_grafico(t,'Tempo-s',X(2,:)','Velocidade do Carrinho - m/s',2,'Velocidade do Carrinho vs. Tempo')
% subplot(2,2,4)
% plotar_um_grafico(t,'Tempo-s',X(4,:)','Velocidade angular da Haste - rad/s',2,'Velocidade Angular da Haste vs. Tempo')

%% Plotandos as Saídas
% subplot(2,2,1)
% plotar_um_grafico(t,'Tempo-s',Y(1,:)','Posição do Carrinho - m',2,'Posição do Carrinho vs. Tempo ')
% subplot(2,2,2)
% plotar_um_grafico(t,'Tempo-s',(180/pi)*Y(2,:)','Ângulo da Haste - º',2,'Ângulo da Haste vs. Tempo')
% subplot(2,2,3)
% plotar_um_grafico(t,'Tempo-s',X(2,:)','Velocidade do Carrinho - m/s',2,'Velocidade do Carrinho vs. Tempo')
% subplot(2,2,4)
% plotar_um_grafico(t,'Tempo-s',X(4,:)','Velocidade angular da Haste - rad/s',2,'Velocidade Angular da Haste vs. Tempo')
...
% 
% subplot(3,2,1)
% plotar_dois_graficos(t,Y(1,:)',t,Yhat(1,:)',2,'Tempo - s','Posição do Carrinho - m', 'Posição do Carrinho vs.Tempo','Verdadeira','Estimada')
% 
% yticks([0 50 100])
% yticklabels({'y = 0','y = 50','y = 100'})
% 
% subplot(3,2,2)
% plotar_dois_graficos(t,(180/pi)*Y(2,:)',t,(180/pi)*Yhat(2,:)',2,'Tempo - s','Ângulo da Haste - º', 'Ângulo da Haste vs.Tempo','Verdadeiro','Estimado')
% subplot(3,2,3)
% 
% p2 = plot(t,X(2,:)','LineWidth',2);
% hold on
% p3 = plot(t,X(4,:)','LineWidth',2);
% p4 = plot(t,Xhat(2,:)','LineWidth',2);
% p5 = plot(t,Xhat(4,:)','LineWidth',2);
% hold off
% legend('Velocidade do Carrinho - Real (m/s)','Velocidade da Haste - Real (rad/s)','Velocidade do Carrinho - Estimada (m/s)','Velocidade da Haste - Estimada (rad/s) ')
% title('Velocidade vs. Tempo')
% ylabel('Velocidades')
% xlabel('Tempo - s')
% grid on;
% 
% % plotar_dois_graficos(t,X(2,:)',t,Xhat(2,:)',2,'Tempo - s','Velocidade do Carrinho - m/s', 'Velocidade do Carrinho vs.Tempo','Verdadeira','Estimada')
% 
% subplot(3,2,4)
% plotar_um_grafico(t,'Tempo-s',U','Sinal de Controle',2,'Sinal de Controle vs. Tempo')
% % plotar_dois_graficos(t,X(4,:)',t,Xhat(4,:)',2,'Tempo - s','Velocidade Angular da Haste - rad/s', 'Ângulo da Haste vs.Tempo','Verdadeira','Estimada')
% 
% subplot(3,2,5)
% p5 = plot(t,Px');
% legend('Posição do Carrinho','Velocidade do Carrinho','Ângulo da Haste','Velocidade da Haste')
% set(p5,'LineWidth',2);
% title('Covarância vs. Tempo')
% ylabel('Covarância')
% xlabel('Tempo - s')
% grid on;
% 
% subplot(3,2,6)
% p6 = plot(t,Yt');
% legend('Posição do Carrinho','Ângulo da Haste')
% set(p6,'LineWidth',2);
% title('Erro de Estimativa vs. Tempo')
% ylabel('Erro')
% xlabel('Tempo - s')
% grid on;
...
% plotar_um_grafico(t,'Tempo-s',(180/pi)*Y(2,:)','Ângulo da Haste - º',2,'Ângulo da Haste vs. Tempo')
% subplot(2,2,3)
% % plotar_dois_graficos(x_data1,y_data1,x_data2,y_data2,width_linha,x_label,y_label,titulo,legenda_1,legenda_2)
% 
% plotar_dois_graficos(t,X(2,:)',t,X(4,:)',2,'Tempo-s','Velocidades - Carrinho(m/s) e Haste(rad/s)','Velocidades vs. Tempo','Carrinho','Haste')
% subplot(2,2,4)
% plotar_um_grafico(t,'Tempo-s',U','Sinal de Controle',2,'Sinal de Controle vs. Tempo')


%% Plotandos os Estados (Esperança e Sistema Real)
% subplot(1,2,1)
% plotar_dois_graficos(t,Y2(1,:)',t,Y(1,:)',.5,'Tempo - s', 'Posição do Carrinho - m', 'Posição do Carrinho vs. Tempo', 'Real (Estocástico','Esperança (Média)')
% subplot(1,2,2)
% plotar_dois_graficos(t,Y2(2,:)',t,Y(2,:)',.5,'Tempo - s', 'Ângulo da Haste - º', 'Ângulo da Haste vs. Tempo', 'Real (Estocástico','Esperança (Média)')

% function graphs = plotar_dois_graficos(x_data1,y_data1,x_data2,y_data2,width_linha,x_label,y_label,titulo,legenda_1,legenda_2)

% plotar_um_grafico(t,'Tempo-s',Y(2,:)','Estados',2,'Estados vs. Tempo')
